...
public class GridDrawer {
	private static final double SHIFT_TO_CENTER = 0.5;
	private Grid myGrid;
	...
	private Shape drawTriangle(int x, int y) {
		...
		s.setTranslateX(x * SHIFT_TO_CENTER);
		s.setTranslateY(y * h);
	...
	private Shape drawSquare(int x, int y) {
		Shape s = new Polygon(new double[] { 0.5, 0.5, -0.5, 0.5, -0.5, -0.5,
				0.5, -0.5 });
		s.setTranslateX(x + SHIFT_TO_CENTER);
		s.setTranslateY(y + SHIFT_TO_CENTER);
		return s;
	}

	private Shape drawHexagon(int x, int y) {
		double h = 0.5 * Math.cos(Math.PI / 6.0);
		Shape s = new Polygon(new double[] { 0.5, 0, 0.25, h, -0.25, h, -0.5,
				0, -0.25, -h, 0.25, -h });
		s.setTranslateX(x * 0.75 + SHIFT_TO_CENTER);
